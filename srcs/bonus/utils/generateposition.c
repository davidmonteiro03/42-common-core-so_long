...
/*   Updated: 2025/02/14 17:55:24 by dcaetano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pos	ft_generateutil(t_map map, t_pos player)
{
	...
	if (range.x > 1)
		ret.x = ft_generateutil2(ret.x, player.x, range.x,
				map.true_pos[START].x);
	if (range.y > 1)
		ret.y = ft_generateutil2(ret.y, player.y, range.y,
				map.true_pos[START].y);
	return (ret);
}
...
t_pos	ft_generateposition2(t_map map, t_pos player)
{
	...
	while ((map.content[tmp.x][tmp.y] != EMPTY \
		&& map.content[tmp.x][tmp.y] != COLLECT) || (tmp.x == player.x \
			&& tmp.y == player.y))
		tmp = ft_generateutil(map, player);
	return (tmp);
}
...
t_pos	ft_generateposition(void)
{
	...
	ret.x = rand() % 3 - 1;
	ret.y = rand() % 3 - 1;
	while ((ret.x == 0 && ret.y == 0) || (ret.x != 0 && ret.y != 0))
	{
		ret.x = rand() % 3 - 1;
		ret.y = rand() % 3 - 1;
	}
