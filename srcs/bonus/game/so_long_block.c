...
/*   Updated: 2025/02/14 17:48:53 by dcaetano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	so_long_put_block(t_game *game, t_image *image, t_pos pos, char *c)
{
	...
	if (*c == COLLECT)
		so_long_draw_block(image,
			game->textures.collectible[game->sprites.frames[COL]
			% COLLECT_FRAMES], pos, true);
	else if (*c == STATIC_ENEMY)
		so_long_draw_block(image,
			game->textures.enemy_static[game->sprites.frames[SEN]
			% ENEMY_STATIC_FRAMES], pos, true);
	else if (*c == MOVING_ENEMY)
		so_long_draw_block(image,
			game->textures.enemy_moving[game->sprites.frames[MEN]
			% ENEMY_MOVING_FRAMES], pos, true);
	else if (*c == FIREBALL)
		so_long_draw_block(image,
			game->textures.fireball[game->sprites.frames[FRB]
			% FIREBALL_FRAMES], pos, true);
}
